...
import 'package:material_kit_flutter/model/acopio_model.dart';
...
class DetailsPageAcopio extends StatefulWidget {
  DetailsPageAcopio({Key key}) : super(key: key);

  @override
  State<DetailsPageAcopio> createState() => _DetailsPageState();
}

class _DetailsPageState extends State<DetailsPageAcopio> {
  File _imageFile;
  ...
  Future _takeScreenshot() async {
    ...
    print(name);
    final imagePath = await File('${directory.path}/$name.png').create();
  ...
  Widget build(BuildContext context) {
    final Acopio recicladora =
        ModalRoute.of(context).settings.arguments as Acopio;
  ...
  Widget _header(Acopio recicladora) {
    return Row(
      children: [
        Container(
  ...
  Widget _Row1(Acopio recicladora) {
    return Table(
      children: [
